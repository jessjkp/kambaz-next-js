            ...
            <input type="checkbox" defaultChecked={todo.completed} className="form-check-input me-2 float-start"
              ...
              {!todo.editing ? ( todo.title ) : (
                <FormControl
                    ...
                    onKeyDown={(e) => {
                        if (e.key === "Enter") {
                        updateTodo({ ...todo, editing: false, title: (e.target as HTMLInputElement).value });
                        }
                    }}
                    />
            )}
                <FaTrash onClick={() => deleteTodo(todo)} className="text-danger float-end mt-1" id="wd-delete-todo"/>
